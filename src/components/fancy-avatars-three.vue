...
<script>
// import * as this.THREE from 'three';
...
export default {
  ...
  mounted() {
    if (this.checkWebGL()) {
      import(/* webpackChunkName: "js/three" */ 'three').then((module) => {
        this.THREE = module;
        this.setup();
        this.initAvatars();
        this.animate();
      ...
      alert('你的浏览不支持3D渲染，请改用chrome浏览器打开');
    }
  },
  watch: {
    type: 'requestUpdate',
  ...
  methods: {
    ...
    setup() {
      ...
      this.camera = new this.THREE.PerspectiveCamera(60,
        window.innerWidth / window.innerHeight, 1, 2000);
      this.camera.position.z = 1500;
      this.scene.fog = new this.THREE.Fog(0xffffff, 1600, 2000);
    ...
    initAvatars() {
      ...
      this.avatarImages = defaults.map(item => new this.THREE.TextureLoader().load(item));
    ...
    initPlaneGroup() {
      ...
      for (let i = 0; i < this.amount; i += 1) {
        ...
        const material = new this.THREE.SpriteMaterial({ map: this.avatarImages[index] });
        const avatar = new this.THREE.Sprite(material);
